...
const resourceName: string = 'columns';

@Injectable()
export class ColumnsService {
  private readonly filePath = path.join(process.cwd(), 'columns.json');
  ...
  findAll() {
    try {
      ...
      console.error('Error reading columns data', e);
      return [];
    }
  }
  ...
  create(item: any) {
    try {
      ...
      console.error('Error writing columns data', e);
      throw e;
    }
  }
  ...
  remove(id: string) {
    try {
      ...
      console.error('Error deleting columns item', e);
