...
export default function Page() {
  ...
  const handleAdd = async () => {
    const val = prompt("Enter keyword trigger:");
    if (!val) return;
    ...
    try {
      const response = await fetch(`${API_URL}/faq`, {
        ...
        body: JSON.stringify({ 
          trigger: val,
          reply: "Thank you for your query. Our agent will contact you shortly."
        }),
      });
      if (response.ok) {
        alert("FAQ Bot added successfully!");
        fetchData();
      } else {
        alert("Failed to add.");
      }
  ...
  return (
    <div className="flex flex-col gap-6 h-full p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white">FAQ Bot</h2>
          <p className="text-sm text-zinc-400">Define frequently asked questions and automated support keyword triggers</p>
        </div>
          ...
          <table className="w-full text-sm text-left text-zinc-400">
            <thead className="text-xs text-zinc-300 uppercase bg-zinc-800/50 border-b border-zinc-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Automated Reply</th>
