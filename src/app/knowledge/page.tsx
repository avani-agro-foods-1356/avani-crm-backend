...
export default function Page() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/knowledge');
      if (response.ok) {
  ...
  const handleAdd = async () => {
    const val = prompt("Enter article title:");
    if (!val) return;
    ...
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/knowledge', {
        ...
        body: JSON.stringify({ 
          title: val,
          content: "This article outlines key eligibility criteria for Personal and Home loans."
        }),
      });
      if (response.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`https://workplace-kay-exchanges-psi.trycloudflare.com/knowledge/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      }
    } catch (error) {
    ...
    <div className="flex flex-col gap-6 h-full p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-400">Store company policies, loan criteria, and training materials for AI</p>
        </div>
          ...
          <table className="w-full text-sm text-left text-zinc-400">
            <thead className="text-xs text-zinc-300 uppercase bg-zinc-800/50 border-b border-zinc-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Content Outline</th>
            ...
            <tbody>
              ...
              {items.map((item) => (
                <tr key={item.id} className="border-b border-zinc-800 hover:bg-zinc-800/30 transition-colors">
                  <td className="px-6 py-4 font-medium text-zinc-200">{item.title || '-'}</td>
                  <td className="px-6 py-4 font-medium text-zinc-200">{item.content || '-'}</td>
