...
export default function Page() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/forms');
      if (response.ok) {
  ...
  const handleAdd = async () => {
    const val = prompt("Enter form name:");
    if (!val) return;
    ...
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/forms', {
        ...
        body: JSON.stringify({ 
          name: val,
          fields: "name, phone, income, profession"
        }),
      });
      if (response.ok) {
        alert("WhatsApp Forms added successfully!");
        fetchData();
      } else {
        alert("Failed to add.");
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`https://workplace-kay-exchanges-psi.trycloudflare.com/forms/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-6 h-full p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white">WhatsApp Forms</h2>
          <p className="text-sm text-zinc-400">Build interactive collection forms for WhatsApp chat</p>
        </div>
          ...
          <table className="w-full text-sm text-left text-zinc-400">
            <thead className="text-xs text-zinc-300 uppercase bg-zinc-800/50 border-b border-zinc-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Fields Schema</th>
            ...
            <tbody>
              ...
              {items.map((item) => (
                <tr key={item.id} className="border-b border-zinc-800 hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-zinc-200">{item.fields || '-'}</td>
