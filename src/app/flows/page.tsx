...
export default function Page() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/flows');
      if (response.ok) {
  ...
  const handleAdd = async () => {
    const val = prompt("Enter flow name:");
    if (!val) return;
    ...
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/flows', {
        ...
        body: JSON.stringify({ 
          name: val,
          description: "Interactive WhatsApp flow steps"
        }),
      });
      if (response.ok) {
        alert("WhatsApp Flows added successfully!");
        fetchData();
      } else {
        alert("Failed to add.");
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`https://workplace-kay-exchanges-psi.trycloudflare.com/flows/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-6 h-full p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white">WhatsApp Flows</h2>
          <p className="text-sm text-zinc-400">Build multi-step conversational menus and interactive flows</p>
        </div>
            ...
            <tbody>
              ...
              {items.map((item) => (
                <tr key={item.id} className="border-b border-zinc-800 hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-zinc-200">{item.description || '-'}</td>
