...
export default function Page() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/campaigns');
      if (response.ok) {
  ...
  const handleAdd = async () => {
    const val = prompt("Enter campaign name:");
    if (!val) return;
    ...
    try {
      ...
      if (response.ok) {
        alert("Campaigns added successfully!");
        fetchData();
      } else {
        alert("Failed to add.");
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`https://workplace-kay-exchanges-psi.trycloudflare.com/campaigns/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      }
    } catch (error) {
    ...
    <div className="flex flex-col gap-6 h-full p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-400">Manage and send WhatsApp broadcast campaigns</p>
        </div>
