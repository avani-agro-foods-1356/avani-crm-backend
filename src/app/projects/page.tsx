...
export default function ProjectsPage() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://workplace-kay-exchanges-psi.trycloudflare.com/projects');
      if (response.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`https://workplace-kay-exchanges-psi.trycloudflare.com/projects/${id}`, { method: 'DELETE' });
      fetchData();
    } catch (error) {
      console.error(error);
    }
