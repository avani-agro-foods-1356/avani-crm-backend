...
const resourceName: string = 'opts';

@Injectable()
export class OptsService {
  private readonly filePath = path.join(process.cwd(), 'opts.json');
  ...
  findAll() {
    try {
      ...
      console.error('Error reading opts data', e);
      return [];
    }
  }
  ...
  create(item: any) {
    try {
      ...
      console.error('Error writing opts data', e);
      throw e;
    }
  }
  ...
  remove(id: string) {
    try {
      ...
      console.error('Error deleting opts item', e);
