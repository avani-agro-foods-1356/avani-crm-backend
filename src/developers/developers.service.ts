...
const resourceName: string = 'developers';

@Injectable()
export class DevelopersService {
  private readonly filePath = path.join(process.cwd(), 'developers.json');
  ...
  findAll() {
    try {
      ...
      console.error('Error reading developers data', e);
      return [];
    }
  }
  ...
  create(item: any) {
    try {
      ...
      console.error('Error writing developers data', e);
      throw e;
    }
  }
  ...
  remove(id: string) {
    try {
      ...
      console.error('Error deleting developers item', e);
