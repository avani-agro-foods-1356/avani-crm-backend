...
const resourceName: string = 'assistant';

@Injectable()
export class AssistantService {
  private readonly filePath = path.join(process.cwd(), 'assistant.json');
  ...
  findAll() {
    try {
      ...
      console.error('Error reading assistant data', e);
      return [];
    }
  }
  ...
  create(item: any) {
    try {
      ...
      console.error('Error writing assistant data', e);
      throw e;
    }
  }
  ...
  remove(id: string) {
    try {
      ...
      console.error('Error deleting assistant item', e);
